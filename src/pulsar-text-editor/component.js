...
// The line-number gutter — emits a single `.gutter.line-numbers` element.
// Wrapped by the editor in a shared `.gutter-container` div so this gutter
// and any custom gutters scroll together via one `translateY(-scrollTop)`.
// Block decorations push lines down in the content area, so the gutter
// inserts matching spacer divs between line numbers to stay aligned.
//
    ...
    >
      ...
      <div style="display: flex; flex-direction: row; flex: 1; overflow: hidden; min-height: 0;">
        ...
        <Show when={showGutter()}>
          {/* Outer .gutter-container clips and provides the scroll
              transform for ALL gutters; inner row lays them out in
              priority order. Custom gutters (git-diff status column,
              etc.) join the row when packages call
              `editor.addGutter({ name, type: 'decoration', … })`. */}
          <div
          ...
          >
            <div
              ref={(el) => { gutterInnerRef = el; component._gutterInnerRef = el; }}
              style={
            ...
            >
              <For each={guttersToRender()}>
                {(gutter) => (
                  gutter.type === 'line-number'
                    ? <GutterContainer
                        scrollTop={scrollTop}
                    ...
                    : <CustomGutter
                        gutter={gutter}
                        decorations={() => customGutterDecorations().get(gutter.name) || []}
                        topForRow={pixelTopForRow}
                        lineHeight={lineHeight}
                        contentHeight={() =>
                          totalScreenRows() * (lineHeight() || 0) + totalBlocksHeight()
                        }
                      />
                )}
              </For>
