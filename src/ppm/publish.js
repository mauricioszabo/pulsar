...
`
    ...
    // Bump package.json's version, commit, and create a `vX.Y.Z` tag —
    // the in-process equivalent of `npm version <bump>`.
    async versionPackage(version) {
      process.stdout.write('Preparing and tagging a new version ');
      try {
        const pack = this.loadMetadata();
        const current = pack.version;
        let next;
        if (['patch', 'minor', 'major', 'premajor', 'preminor', 'prepatch', 'prerelease'].includes(version)) {
          next = semver.inc(current, version);
        } else {
          if (!semver.valid(version)) throw new Error(`Invalid version: ${version}`);
          next = version;
        }
        if (!next) throw new Error(`Could not compute next version from ${current} + ${version}`);
        pack.version = next;
        this.saveMetadata(pack);

        const gitCommand = npmrc.get('git') || 'git';
        const runGit = (args) => new Promise((resolve, reject) => {
          this.spawn(gitCommand, args, (code, stderr, stdout) => {
            if (code === 0) resolve(stdout?.trim());
            else reject(`git ${args.join(' ')} failed: ${(stdout ?? '') + (stderr ?? '')}`.trim());
          });
        });

        const tag = `v${next}`;
        await runGit(['add', 'package.json']);
        await runGit(['commit', '-m', `Prepare v${next} release`]);
        await runGit(['tag', '-a', tag, '-m', `Prepare v${next} release`]);
        this.logSuccess();
        return tag;
      } catch (error) {
        this.logFailure();
